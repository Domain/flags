...
namespace flags
{
    namespace detail
    {
        ...
        struct parser
        {
            ...
            const argument_map& short_options() const { return short_options_; }
            const argument_map& long_options() const { return long_options_; }
        ...
        private:
            ...
            void on_option(const std::string_view& option)
            {
                ...
                current_option_->remove_prefix(1);

                auto is_short = true;
                if (current_option_->at(0) == '-')
                {
                    current_option_->remove_prefix(1);
                    is_short = false;
                }
                ...
                if (is_short)
                {
                    auto all = *current_option_;
                    auto len = current_option_->length();
                    for (size_t i = 0; i < len - 1; i++)
                    {
                        current_option_ = all;
                        current_option_->remove_prefix(i);
                        current_option_->remove_suffix(len - i - 1);
                        flush();
                    }

                    current_option_ = all;
                    current_option_->remove_prefix(len - 1);
                }
            ...
            void on_value(const std::optional<std::string_view>& value = std::nullopt)
            {
                ...
                if (current_option_->length() > 1)
                {
                    long_options_.emplace(*current_option_, value);
                }
                else
                {
                    short_options_.emplace(*current_option_, value);
                }
                current_option_.reset();
            }

            std::optional<std::string_view> current_option_;
            argument_map short_options_;
            argument_map long_options_;
    ...
    struct CArgs
    {
        CArgs(int argc, char** argv) : parser_(argc, argv) {}
        ...
        std::optional<T> get(const std::string_view& option) const
        {
            if (option.length() == 1)
            {
                return detail::get<T>(parser_.short_options(), option);
            }
            return detail::get<T>(parser_.long_options(), option);
        }

        template <class T>
        std::optional<T> get(std::initializer_list<const char*> options) const
        {
            for (auto option : options)
            {
                auto opt = get<T>(option);
                if (opt)
                {
                    return opt;
                }
        ...
        template <class T>
        T get(std::initializer_list<const char*> options, T&& default_value) const
        {
            for (auto option : options)
            {
                auto opt = get<T>(option);
